...
def require_resident_or_admin(f):
    """Décorateur pour restreindre l'accès aux résidents et administrateurs seulement."""
    @functools.wraps(f)
    def decorated_function(*args, **kwargs):
        logger.debug(f"Vérification résident/admin pour {f.__name__}")
        ...
        # Vérifier le rôle (résident ou admin)
        user_role_value = session.get('user_role') or session.get('role')
        logger.debug(f"Role utilisateur: {user_role_value}")
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('login.html',
                                 error_message='Erreur système. Veuillez réessayer.'), 401
...
def create_project():
    ...
    try:
        ...
        total_units = request.form.get('total_units')  # Correction: le champ s'appelle total_units

        # Validation des champs requis
        if not construction_year:
            flash('Erreur: Année de construction requise', 'error')
            return redirect(url_for('projets'))
        if not building_area:
            flash('Erreur: Superficie du bâtiment requise', 'error')
            return redirect(url_for('projets'))
        if not total_units:
        ...
        if building_area_float > land_area:
