#!/usr/bin/env python3
"""
Runner simple pour tous les tests d'acceptance
"""
...
def setup_test_logger():
    ...
    test_logger = logging.getLogger("test_acceptance_runner")
...
def main():
    ...
    logger.info("Découverte des tests d'acceptance...")
    
    # Découverte des tests depuis le répertoire acceptance
    loader = unittest.TestLoader()
    start_dir = os.path.join(os.path.dirname(__file__), 'acceptance')
    ...
    if test_count == 0:
        logger.error("ERREUR: Aucun test d'acceptance trouvé dans tests/acceptance/")
        return 1
    ...
    logger.info("=" * 60)
    logger.info("SOMMAIRE FINAL - TESTS D'ACCEPTANCE")
    logger.info("=" * 60)
    ...
    if all_passed:
        logger.info(f"RÉSULTAT: {status_text} - Tous les tests d'acceptance passent")
    else:
    ...
    if all_passed:
        logger.info("STATUT FINAL: TESTS D'ACCEPTANCE RÉUSSIS")
    else:
        logger.error("STATUT FINAL: TESTS D'ACCEPTANCE ÉCHOUÉS")
