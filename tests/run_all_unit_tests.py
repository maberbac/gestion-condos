#!/usr/bin/env python3
"""
Runner simple pour tous les tests unitaires
"""
...
def main():
    ...
    logger.info("Découverte des tests unitaires...")
    
    # Découverte des tests depuis le répertoire unit
    ...
    if test_count == 0:
        logger.error("ERREUR: Aucun test unitaire trouvé dans tests/unit/")
        return 1
    ...
    summary_logger.info("SOMMAIRE FINAL - TESTS UNITAIRES")
    summary_logger.info("=" * 60)
    ...
    if all_passed:
        summary_logger.info(f"RÉSULTAT: {status_text} - Tous les tests unitaires passent")
    else:
    ...
    if all_passed:
        summary_logger.info("STATUT FINAL: TESTS UNITAIRES RÉUSSIS")
    else:
        summary_logger.error("STATUT FINAL: TESTS UNITAIRES ÉCHOUÉS")
